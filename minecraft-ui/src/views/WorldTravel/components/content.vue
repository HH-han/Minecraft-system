<template>
  <div class="mydestination-all">
    <!-- 调试信息 -->
    <div v-if="true" class="debug-info" style="display: none;">
      <p>加载状态: {{ loading }}</p>
      <p>错误信息: {{ error }}</p>
      <p>国家数量: {{ countries.length }}</p>
      <p>分页目的地数量: {{ paginatedDestinations.length }}</p>
      <p>热门目的地: {{ destinationName }}</p>
    ...
    <div v-else-if="error" class="error-overlay">
      <p>加载失败: {{ error }}</p>
      <button @click="fetchCountries" class="retry-btn">重试</button>
    </div>
    
    <header v-else class="main-section">
      <!-- 主视觉区 -->
    ...
    <main v-if="!loading && !error" class="destination-page">
      <!-- 目的地导航区域 -->
      <div>
        <Destination />
      </div>
...
const fetchCountries = async () => {
  ...
  try {
    ...
    if (formattedCountries.length > 0) {
      destinationName.value = formattedCountries.slice(0, 3).map(c => c.name).join(', ')
    }
...
const getContinentName = (continentId) => {
  ...
  return continentMap[continentId] || '其他'
}
...
const forceUpdate = () => {
  ...
  paginatedDestinations.value = [...paginatedDestinations.value]
}
</script>
